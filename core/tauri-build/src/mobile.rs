...
pub fn generate_gradle_files(project_dir: PathBuf, config: &Config) -> Result<()> {
  ...
  let app_gradle_properties_path = project_dir.join("app").join("gradle.properties");
...
dependencies {"
  ...
  if let Some(version) = config.version.as_ref() {
    app_gradle_properties.push_str(&format!("tauri.android.versionName={}\n", version));
    if let Some(version_code) = config.bundle.android.version_code.as_ref() {
      app_gradle_properties.push_str(&format!("tauri.android.versionCode={}\n", version_code));
  ...
  println!("cargo:rerun-if-changed={}", app_gradle_properties_path.display());
